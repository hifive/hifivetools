...
(function(window, $) {
	...
	function expose(namespace, object) {
		...
		for ( var prop in object) {
			if (object.hasOwnProperty(prop)) {
				if (nsObj[prop]) {
					throw new Error(h5.u.str.format('名前空間"{0}"には、プロパティ"{1}"が既に存在します。', namespace,
							prop));
				}
	...
	var lapInterceptor = function(invocation) {
		...
		var post = function() {
			...
			that.log.info(currentName + ' "' + currentFuncName + '": ' + (endTime - startTime)
					+ "ms");
		};
...
(function(window, $) {
	...
	var escapeJs = function(str) {
		...
		return str.replace(/\\/g, '\\\\').replace(/"/g, '\"').replace(/'/g, "\'").replace(/\//g,
				'\/').replace(/</g, '&#x3c;').replace(/>/g, '&#x3e;').replace(/&#x0d/g, '\r')
				.replace(/&#x0a/g, '\n');
	};
	/**
				...
				if ($.type(ret) === 'string') {
					...
					if (clazz !== undefined && clazz !== '') {
						...
						case 'function':
							...
							var body = value.substring(value.indexOf('{') + 1, value
									.lastIndexOf('}'));
							var argArray = [];
							if (args) {
								argArray = args.split(',');
...
(function(window, $) {
	...
	var isPromise = function(object) {
		return object != null && object['done'] && object['fail'] && !object['resolve']
				&& !object['reject'];
	};
	...
	Order.prototype = {
		...
		parallelAndSetWith: function(functions) {
			...
			for ( var i = 0; i < funcLen; i++) {
				var params = functions[i];
				var key = params[0];
				var of = this._createOrderedFunction(params[1], this._wrap(key, params[2]),
						params[3]);
				array.push(of);
			}
			this.orderFunctions.push(array);
		...
		_execute: function(of) {
			return $.isArray(of.args) ? of.func.apply(of.that, of.args) : of.func.call(of.that,
					of.args);
		},
...
(function(window, $) {
	...
	function readCSSCanvasStyle(cssClass) {
		...
		for ( var prop in readCSSPropertyMap) {
			var elem = $('<div></div>').addClass(cssClass)
					.addClass(canvasCSSClassNamePrefix + prop).appendTo('body');
			readStyles[prop] = {};
	...
	function Indicator(target, option) {
		...
		// BlockUIのスタイル
		var blockUISetting = {
			message: '<span class="indicator-throbber" style="display:inline-block;">'
					+ '</span><span class="indicator-message" style="'
					+ (opts.message === "" ? 'display: none;' : 'display: inline-block;') + '">'
					+ opts.message + '</span>',
			css: {},
			overlayCSS: {},
...
(function(window, $) {
	...
	var ajax = function(var_args) {
		...
		var hasFailCallback = option
				&& (option.error || option.fail || option.complete || option.always);
...
(function($) {
	// #delete end
	var h5Logger = h5.u.createLogger('h5.core.view');
...
(function($) {
	// #delete end
	var h5Logger = h5.u.createLogger('h5.core');
	var getDeferred = h5.async.deferred;
	...
	var internal = {
		...
		checkDelegateCircular: function(object, array) {
			...
			array.push(object);
			for ( var prop in object) {
				if (this.isChildController(prop, object)
						&& !this.checkDelegateCircular(object[prop], array)) {
					return false;
				}
			}
			return true;
			...
			var worker = function(object) {
				...
				for ( var prop in object) {
					if (that.isChildController(prop, object)) {
						...
						var promise = controllerContext ? c.__controllerContext[propertyName]
								: c[propertyName];
						if (promise) {
		...
		bindUseHandlers: function(obj) {
			...
			var execute = function(controller) {
				...
				for ( var prop in controller) {
					...
					if (!c || !that.isChildController(prop, controller)
							|| c.__controllerContext.isRoot) {
						continue;
					}
					execute(c);
				}
		...
		isChildController: function(prop, obj) {
			return endsWith(prop, 'Controller') && prop !== '__rootController'
					&& prop !== '__parentController' && !$.isFunction(obj[prop]);
		},
			...
			var execute = function(obj, isRoot) {
				...
				var func = function() {
					...
					if (lifecycleFunc) {
						...
						ret = init ? obj.__init(initializedContext) : obj
								.__ready(initializedContext);
					}
					var callback = init ? that.createCallbackForInit(obj, isRoot) : that
							.createCallbackForReady(obj, isRoot);
					if (ret && h5.async.isPromise(ret)) {
						ret.done(function() {
							callback();
						});
		...
		getPromisesForInit: function(controller) {
			var templatePromises = this.getInternalControllerPromises(controller,
					'templatePromise', true);
			...
			if (typeof element === 'string') {
				...
				if (this.isOutOfController(selector)) {
					...
					if (isTemplate && this.isCorrectTemplatePrefix(s)) {
						throw new Error(
								'update/append/prependView() の第1引数に"window", "window.", "navigator", "navigator."で始まるセレクタは指定できません。');
					}
		...
		useBindObj: function(bindObj, useBind) {
			...
			this.setupUnbindMap(bindObj.controller, bindObj.selector, bindObj.eventName,
					bindObj.handler);
		...
		normalizeEventObjext: function(event) {
			if (event && event.offsetX == null && event.offsetY == null && event.pageX
					&& event.pageY) {
				var target = event.target;
				if (target.ownerSVGElement) {
		...
		getBindTarget: function(element, rootElement, controller) {
			...
			if (targets.length > 1) {
				throw new Error(format('コントローラ"{0}"のバインド対象となる要素が2つ以上存在します。バインド対象は1つのみにしてください。',
						controller.__name));
			}
	...
	function createController(targetElement, baseObj, param, fwOpt) {
		...
		if (!controllerName || $.trim(controllerName).length === 0) {
			h5Logger.error('コントローラの名前が定義されていません。__nameにコントローラ名を設定して下さい。');
			throw new Error(baseObj);
		}
		// 初期化パラメータがオブジェクトかどうかチェック
		if (param && !$.isPlainObject(param)) {
			h5Logger.error('コントローラ"{0}"の初期化パラメータがオブジェクトではありません。初期化パラメータにはオブジェクトを設定してください。',
					controllerName);
			throw new Error(baseObj);
		}
		if (baseObj.__controllerContext) {
			h5Logger.error(object);
		...
		if (templates && templates.length > 0) {
			...
			templatePromise.done(function(result) {
				if (templates && templates.length > 0) {
					h5Logger.info('コントローラ"{0}"のテンプレートの読み込みに成功しました。', controllerName);
				}
			}).fail(
					function(result) {
						...
						for ( var i = 0, len = faults.length; i < len; i++) {
							h5Logger.error(
									'コントローラ"{0}"のテンプレート(' + faults[i].url + ')の読み込みに失敗しました。',
									controllerName);
						}
					});
		} else {
		...
		if (isGlobalController) {
			...
			GlobalController.prototype.getController = function(rootElement) {
				var target = typeof rootElement === 'string' ? $(rootElement).get(0)
						: rootElement.get ? rootElement.get(0) : rootElement;
			...
			if (internal.isLifecycleEvent(targetObj, prop)) {
				...
				if (internal.isOutOfController(selector)) {
					...
					if (selectTarget === 'this') {
						h5Logger.error(baseObj);
						throw new Error(format('コントローラ"{0}"でセレクタ名にthisが指定されています。'
				...
				if (propObj.bindMap[selector][eventName]) {
					h5Logger.error(baseObj);
					throw new Error(format('"コントローラ"{0}"で、{1} {2}"というイベントハンドラが重複して設定されています。',
							targetObj.__name, selector, eventName));
				}
				...
				if (!internal.checkDelegateCircular(controllerTarget, [targetObj])) {
					throw new Error(format('コントローラ"{0}"で、参照が循環しているため、コントローラを生成できません。',
							targetObj.__name));
				}
				var c = createController(null, $.extend(true, {}, targetObj[prop]), param, $
						.extend({
							isInternal: true,
							eventEmulation: eventEmulation
						}, fwOpt));
				...
				if (!internal.checkLogicCircular(logicTarget, [])) {
					throw new Error(format('コントローラ"{0}"のロジック"{1}"で、参照が循環しているため、ロジックを生成できません。',
							targetObj.__name, logicTarget.__name));
				}
		...
		if (meta) {
			for ( var prop in meta) {
				...
				var c = propObj[prop];
				if (c === undefined) {
					h5Logger.error('コントローラ"{0}"には__metaで指定されたプロパティ"{1}"がありません。', propObj.__name,
							prop);
					throw new Error(original);
				}
				if (c === null) {
					h5Logger.error('コントローラ"{0}"の__metaに指定されたキーがnullです。'
							+ 'コントローラを持つプロパティキー名を指定してください。', propObj.__name, prop);
					throw new Error(original);
				}
				if (Controller.prototype.constructor === c.constructor) {
					h5Logger.error('コントローラ"{0}"の__metaに指定されたキー"{1}"の値はコントローラではありません。'
							+ 'コントローラを持つプロパティキー名を指定してください。', propObj.__name, prop);
					throw new Error(original);
				}
			}
...
(function($) {
	// #delete end
	var h5Logger = h5.u.createLogger('h5.api.appcache');
