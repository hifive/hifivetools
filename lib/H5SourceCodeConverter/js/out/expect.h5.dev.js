 ...
 * hifive
 *   version 1.0.4
 *   build at 2012/07/13 21:03:55.334 (+0900)
...
(function($) {
  ...
  //h5存在チェック
  if (window.h5) {
    if (window.h5.env && (window.h5.env.version === '1.0.4')) {
  ...
  function throwFwError(code, msgParam, detail) {
    var e = new Error();
    if (code) {
      e.code = code;      // TODO codeは必須、ない場合は…throw Error
    }
      ...
      function existStack(obj) {
        ...
        switch (type) {
          ...
          case 'array':
            ...
            if (hash) {
              ret += ((val.length) ? ',' : '') + '"' + typeToCode('objElem') + '{' + hash.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
              ret = ret.replace(/,$/, '');
              ret += '}"';
            }
            ret += ']';
      ...
      _traceFunctionName: function(fn) {
        ...
        if (errMsg) {
          ...
          traces = $.map(traces, function(value) {
            if (value.length === 0) {
              ret = null;              // 不要なデータ(Chromeは配列の先頭, FireFoxは配列の末尾に存在する)
            } else if ($.trim(value) === '') {
              ret = '{anonymous}';              // ログとして出力されたが関数名が無い
            } else {
              ret = $.trim(value);
  ...
  (function() {
    ...
    function createAndBindController(targetElement, controllerDefObj, param, fwOpt) {
      ...
      controller.initPromise = controller.__controllerContext.initDfd.promise().fail(dummyFailHandler);
      controller.__controllerContext.readyDfd = getDeferred();
      controller.readyPromise = controller.__controllerContext.readyDfd.promise();
      if (!isRoot) {
    ...
    var cacheManager = {
      ...
      getTemplateByUrls: function(resourcePaths) {
        ...
        function load(absolutePath, filePath, df) {
          h5.ajax(filePath).done(function(result, statusText, obj) {
            ...
            // アクセス中のURLのプロミスを保持するaccessingUrlsから、このURLのプロミスを削除する
            delete that.accessingUrls[absolutePath];
            df.reject(createRejectReason(ERR_CODE_TEMPLATE_AJAX, [e.status, absolutePath], {
              url: absolutePath,
              path: filePath,
              error: e
            }));
  ...
  (function() {
    ...
    function Indicator(target, option) {
      ...
      var blockUISetting = {
        ...
        centerX: false,
        centerY: false,
        onUnblock: function() {          // blockUIが、画面ブロックの削除時に実行するコールバック関数
  ...
  (function() {
    ...
    h5.u.obj.expose('h5.ui.jqm.manager', {
      ...
      init: function() {
        ...
        initCalled = true;
        $(function() {
          jqmControllerInstance = h5internal.core.controllerInternal('body', jqmController, null, {
            managed: false
          });
          bindToActivePage();
        });
      ...
      define: function(id, cssSrc, controllerDefObject, initParam) {
        ...
        if ($.mobile.activePage && $.mobile.activePage.attr('id') === id && jqmControllerInstance) {
          bindToActivePage();
        } else {
          this.init();
        }
