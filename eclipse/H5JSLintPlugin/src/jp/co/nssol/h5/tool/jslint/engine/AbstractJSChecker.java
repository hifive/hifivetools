...
public abstract class AbstractJSChecker implements JSChecker {
	 ...
	 * Rhinoの最適化レベル.(-1~9).
	 */
	private static final int OPTIMAZATION_LEVEL = 9;
	...
	Scriptable optionsAsJavaScriptObject() {
		...
		return (Scriptable) factory.call(new ContextAction() {
			public Object run(Context cx) {
				...
				// for (JSLintOptions option : JSLintOptions.values()) {
				// putOpts(opts, option.getKey(),
				// property.getProperty(option.getKey()), option.getClazz());
				// }
